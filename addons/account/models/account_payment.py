...
class account_payment(models.Model):
    ...
    def validate_from_invoice(self):
        ...
        if invoice.payment_term_id and self.split_payment:
            totlines = invoice.payment_term_id.with_context(currency_id=self.env.user.company_id.id).compute(amount, invoice.date_invoice)
            payment_to_create = []
            for i, t in enumerate(totlines):
                if i == 0:
                    self.write(get_line(t[1], t[0]))
                    self.post()
                else:
...
class payment_register(models.TransientModel):
    ...
    def _prepare_payment_vals(self, invoice):
        ...
        if invoice.payment_term_id:
            ...
            for i, t in enumerate(totlines):
                values.append(get_line(t[1], t[0]))
        else:
            values.append(get_line(abs(amount), self.payment_date))
        return values
