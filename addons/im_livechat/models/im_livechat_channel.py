...
class ImLivechatChannel(models.Model):
    ...
    def _get_mail_channel(self, anonymous_name, previous_operator_id=None, user_id=None, country_id=None):
        ...
        if user_id:
            ...
            if current_user and current_user.active:  # valid session user (not public)
                channel_partner_to_add.append((4, current_user.partner_id.id))
        ...
        mail_channel = self.env["mail.channel"].with_context(mail_create_nosubscribe=False).sudo().create({
            ...
            'name': ', '.join([current_user.name if user_id else anonymous_name, operator.livechat_username if operator.livechat_username else operator.name]),
            'public': 'private',
            'email_send': False,
        })
