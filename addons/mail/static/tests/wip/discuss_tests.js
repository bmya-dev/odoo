...
QUnit.test('sidebar quick search', async function (assert) {
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root > .o_sidebar > .o_group.o_channel > .o_list > .o_item').length, 20, "should have 20 channel items");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root > .o_sidebar > input.o_quick_search').length, 1, "should have quick search in sidebar");

    const quickSearch = document.querySelector('.o_mail_wip_discuss_root > .o_sidebar > input.o_quick_search');
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root > .o_sidebar > .o_group.o_channel > .o_list > .o_item').length, 11, "should have filtered to 11 channel items");
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root > .o_sidebar > .o_group.o_channel > .o_list > .o_item').length, 1, "should have filtered to a single channel item");
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss_root > .o_sidebar > .o_group.o_channel > .o_list > .o_item').dataset.threadLocalId, 'mail.channel_12', "should have filtered to a single channel item with ID 12");
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root > .o_sidebar > .o_group.o_channel > .o_list > .o_item').length, 0, "should have filtered to no channel item");
});
...
QUnit.test('basic control panel rendering', async function (assert) {
    ...
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss > .o_cp_controller > .o_control_panel > .breadcrumb').textContent, "Inbox", "display inbox in the breadcrumb");
    const markAllReadButton = document.querySelector('.o_mail_wip_discuss > .o_cp_controller > .o_control_panel > .o_cp_left > .o_cp_buttons > div > button.o_mark_all_read');
    ...
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss > .o_cp_controller > .o_control_panel > .breadcrumb').textContent, "Starred", "display starred in the breadcrumb");
    const unstarAllButton = document.querySelector('.o_mail_wip_discuss > .o_cp_controller > .o_control_panel > .o_cp_left > .o_cp_buttons > div > button.o_unstar_all');
    ...
    await testUtils.dom.click(document.querySelector('.o_mail_wip_discuss_root .o_item[data-thread-local-id="mail.channel_20"]'));

    assert.strictEqual(document.querySelector('.o_mail_wip_discuss > .o_cp_controller > .o_control_panel > .breadcrumb').textContent, "#General", "display general in the breadcrumb");
    const inviteButton = document.querySelector('.o_mail_wip_discuss > .o_cp_controller > .o_control_panel > .o_cp_left > .o_cp_buttons > div > button.o_invite');
    assert.isVisible(inviteButton, "should have visible button 'Invite' in the control panel of channel");
});

QUnit.test('inbox: mark all message as read', async function (assert) {
    assert.expect(8);
    ...
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss_root .o_item[data-thread-local-id="mail.box_inbox"] > .o_counter').textContent, "2", "inbox should have counter of 2");
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss_root .o_item[data-thread-local-id="mail.channel_20"] > .o_counter').textContent, "2", "channel should have counter of 2");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root .o_message_list > .o_message').length, 2, "should have 2 messages in inbox");
    let markAllReadButton = document.querySelector('.o_mail_wip_discuss > .o_cp_controller > .o_control_panel > .o_cp_left > .o_cp_buttons > div > button.o_mark_all_read');
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root .o_item[data-thread-local-id="mail.box_inbox"] > .o_counter').length, 0, "inbox should display no counter (= 0)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root .o_item[data-thread-local-id="mail.channel_20"] > .o_counter').length, 0, "channel should display no counter (= 0)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root .o_message_list > .o_message').length, 0, "should have no message in inbox");
    markAllReadButton = document.querySelector('.o_mail_wip_discuss > .o_cp_controller > .o_control_panel > .o_cp_left > .o_cp_buttons > div > button.o_mark_all_read');
    assert.ok(markAllReadButton.disabled, "should have disabled button 'Mark all read' in the control panel of inbox (no messages)");
});
...
QUnit.test('starred: unstar all', async function (assert) {
    ...
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss_root .o_item[data-thread-local-id="mail.box_starred"] > .o_counter').textContent, "2", "starred should have counter of 2");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root .o_message_list > .o_message').length, 2, "should have 2 messages in starred");
    let unstarAllButton = document.querySelector('.o_mail_wip_discuss > .o_cp_controller > .o_control_panel > .o_cp_left > .o_cp_buttons > div > button.o_unstar_all');
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root .o_item[data-thread-local-id="mail.box_starred"] > .o_counter').length, 0, "starred should display no counter (= 0)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root .o_message_list > .o_message').length, 0, "should have no message in starred");
    unstarAllButton = document.querySelector('.o_mail_wip_discuss > .o_cp_controller > .o_control_panel > .o_cp_left > .o_cp_buttons > div > button.o_unstar_all');
    assert.ok(unstarAllButton.disabled, "should have disabled button 'Unstar all' in the control panel of starred (no messages)");
});
...
QUnit.test('toggle_star message', async function (assert) {
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root .o_item[data-thread-local-id="mail.box_starred"] > .o_counter').length, 0, "starred should display no counter (= 0)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root .o_message_list > .o_message').length, 1, "should have 1 message in channel");
    let message = document.querySelector('.o_mail_wip_discuss_root .o_message_list > .o_message');
    ...
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss_root .o_item[data-thread-local-id="mail.box_starred"] > .o_counter').textContent, "1", "starred should display a counter of 1");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root .o_message_list > .o_message').length, 1, "should have kept 1 message in channel");
    message = document.querySelector('.o_mail_wip_discuss_root .o_message_list > .o_message');
    ...
    assert.verifySteps(['rpc:toggle_message_starred']);
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root .o_item[data-thread-local-id="mail.box_starred"] > .o_counter').length, 0, "starred should no longer display a counter (= 0)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss_root .o_message_list > .o_message').length, 1, "should still have 1 message in channel");
    message = document.querySelector('.o_mail_wip_discuss_root .o_message_list > .o_message');
