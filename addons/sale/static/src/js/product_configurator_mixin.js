...
var ProductConfiguratorMixin = {
    ...
    onChangeVariant: function (ev) {
        ...
        this._throttledGetCombinationInfo(this._getUniqueId($parent))(ev);
    },
    /**
     * @see onChangeVariant
     *
    ...
    _getCombinationInfo: function (ev) {
        ...
        var $parent = $(ev.target).closest('.js_product');
        var uniqueId = self._getUniqueId($parent);

        // Guarantee to show the info for the last selected combination only.
        // Needs to be saved per uniqueId because on the configurator modal
        // there are several elements potentially calling this method.
        this.dropPrevious = this.dropPrevious || {};
        this.dropPrevious[uniqueId] = this.dropPrevious[uniqueId] || new concurrency.DropPrevious();
        ...
        return this.dropPrevious[uniqueId].add(ajax.jsonRpc(this._getUri('/product_configurator/get_combination_info'), 'call', {
     ...
     * call, and which is memoized per `uniqueId`.
     *
    ...
    _throttledGetCombinationInfo: _.memoize(function (uniqueId) {
        return _.throttle(this._getCombinationInfo.bind(this), 500);
    }),
    /**
