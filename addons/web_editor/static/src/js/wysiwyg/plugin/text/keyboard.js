...
var KeyboardPlugin = AbstractPlugin.extend({
    ...
    handleTab: function (untab) {
        ...
        if (range.scArch.isText()) { // Clean up start spaces on textNode
            range.scArch.nodeValue.replace(startSpace, function (startSpaces) {
                range.so = startSpaces.length === range.so ? 0 : range.so;
                return '';
            });
        }
        return false;
        if (range.scArch.isLeftEdgeOfBlock() || range.scArch.isEmpty()) {
    ...
    _isOnLeftEdgeOfCell: function () {
        var range = this.dependencies.Arch.getRange();
        if (!range.isCollapsed()) {
            return false;
        }
        var archNode = range.scArch.nextUntil(range.scArch.isNotVirtual);
        var tdAncestor = archNode.ancestor(archNode.isCell);
        return archNode && tdAncestor && archNode.isLeftEdgeOf(tdAncestor);
    },
    _isOnLeftEdgeOfLi: function () {
        var range = this.dependencies.Arch.getRange();
        if (!range.isCollapsed()) {
            return false;
        }
        var archNode = range.scArch.nextUntil(range.scArch.isNotVirtual);
        var liAncestor = archNode.ancestor(archNode.isLi);
        return archNode && liAncestor && archNode.isLeftEdgeOf(liAncestor);
    },
    /**
    ...
    _onBackspace: function (e) {
        if (this._isOnLeftEdgeOfLi()) {
            this.dependencies.Arch.outdent(); // Outdent if on left edge of an indented block
        } else if (!this._isOnLeftEdgeOfCell()) { // Do nothing if on left edge of a table cell
    ...
    _onEnter: function (e) {
        if (e.shiftKey) {
            ...
            var range = this.dependencies.Arch.getRange();
            var liAncestor = range.scArch.isLi();
