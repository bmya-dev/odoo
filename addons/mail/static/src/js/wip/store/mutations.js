...
const mutations = {
    ...
    'discuss/update'({ state }, changes) {
        ...
        } else if ('$thread' in changes && changes.$thread !== state.discuss.$thread) {
            shouldRecomputeStringifiedDomain = true;
        }
        if ('open' in changes && !changes.open) {
            state.discuss.$thread = undefined;
        }
        ...
        const thread = state.threads[state.discuss.$thread];
        if (!thread) {
            return;
        }
    ...
    'message/create'(
        ...
        if (authorPartnerID) {
            const $partner = commit('partner/insert', {
                id: authorPartnerID,
                name: authorName,
            });
            commit('partner/link_message', { $message, $partner });
        }
        ...
        for (let $thread of message.$threads) {
            ...
            if (!state.threadCaches[$threadCache]) {
                commit('thread_cache/create', { $thread });
            }
    ...
    'notification/partner/toggle_star'(
        ...
        if (!state.threadCaches[$starredBoxMainCache]) {
            commit('thread_cache/create', { $thread: $starredBox });
        }
        for (let messageID of message_ids) {
            let $message = `mail.message_${messageID}`;
    ...
    'partner/create'({ set, state }, data) {
        ...
        const $partner = partner.localID;
        if (state.partners[$partner]) {
            console.warn(`partner with local ID "${$partner}" already exists in store`);
            return;
        }
        set(state.partners, $partner, partner);
        // todo: links
        return $partner;
    },
    /**
    ...
    'thread/link_thread_cache'({ commit, state }, { $thread, $threadCache }) {
        if (!state.threads[$thread]) {
            // todo: this is bad to determine model and id from spliting $thread...
