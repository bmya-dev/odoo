...
var Selector = AbstractPlugin.extend({
    ...
    search: function (archNode, string, options) {
        ...
        if (typeof archNode === 'string') {
            options = string;
            string = archNode;
            archNode = this.dependencies.Arch._archClone;
        }

        if (!(archNode instanceof ArchNode)) {
            archNode = this.dependencies.Arch.getNode(archNode) || this.dependencies.Arch._parseElement(archNode);
        }
    ...
    _searchFromToken_PSEUDO_not: function (archNodes, value) {
        var self = this;
        console.log('not', value, archNodes);
        return this._searchFromTokenLoop(archNodes, function (archNode) {
