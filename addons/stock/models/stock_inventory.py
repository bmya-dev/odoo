...
class Inventory(models.Model):
    ...
    def action_start(self):
        ...
        return self[0].action_open_inventory_lines()

    def _action_start(self):
        """ Confirms the Inventory Adjustment and generates its inventory lines
            if its state is draft and don't have already inventory lines (can
            happen with demo data or test).
        """
        self.ensure_one()
        if self.state == 'draft':
            vals = {
...
class InventoryLine(models.Model):
    ...
    def _domain_location_id(self):
        ...
        if self._context.get('default_inventory_id', False):
            inventory = self.env['stock.inventory'].browse(self._context.get('default_inventory_id'))
            if inventory.exists():
