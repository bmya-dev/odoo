...
const actions = {
    ...
    async 'document_thread/load'(
        ...
        const $messages = msgsData.map(data => commit('message/create', { ...data }));
    ...
    async 'notification/channel/message'(
        ...
        const $thread = `mail.channel_${channelID}`;
        const thread = state.threads[$thread];
    ...
    async 'thread/load'(
        ...
        } else if ($thread === 'mail.box_inbox') {
            domain = domain.concat([['needaction', '=', true]]);
        } else if ($thread === 'mail.box_starred') {
            domain = domain.concat([['starred', '=', true]]);
        } else if ($thread === 'mail.box_moderation') {
            domain = domain.concat([['need_moderation', '=', true]]);
        }
        commit('thread_cache/update', { $threadCache, changes: { loading: true } });
    ...
    async 'thread/load_more'(
        ...
        commit('thread_cache/update', { $threadCache, changes: { loadingMore: true } });
        const minMessageID = Math.min(
            ...threadCache.$messages.map($message => state.messages[$message].id));
