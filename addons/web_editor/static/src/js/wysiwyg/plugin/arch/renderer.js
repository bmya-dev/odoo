 ...
 * @param {Object} options.attributeBlacklist
 **/
function Renderer (editable, options) {
    this.editable = editable;
    this.options = options || {};
    if (this.options.attributeBlacklist) {
        ...
        if (json.isVirtual && !options.keepVirtual) {
            ...
            if (changes.attributes) {
                changes.attributes.forEach(function (attribute) {
                    if (!attribute[1] || !attribute[1].length || self.options.attributeBlacklist.indexOf(attribute[0]) !== -1) {
