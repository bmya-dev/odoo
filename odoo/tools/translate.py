    ...
    def __init__(self, source):
        ...
        def get_pot_path(source_name):
            ...
            if isinstance(source_name, str) and source_name.endswith('.po'):
                ...
                addons_module_i18n, _ignored = os.path.split(source_name)
                ...
                return os.path.exists(pot_path) and pot_path or False
            return False

        # polib accepts either a path, either the file content as a string
        # NamedTemporaryFile is not a BufferedIOBase but a wrapper
        if isinstance(source, io.BufferedIOBase) or \
            (hasattr(source, 'file') and isinstance(source.file, io.BufferedIOBase)):
