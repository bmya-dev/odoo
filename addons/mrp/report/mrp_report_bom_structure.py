        ...
        for bom_id in docids:
            ...
            for product_variant_id in candidates:
                if data and data.get('childs'):
                    ...
                    doc = self._get_pdf_line(bom_id, product_id=product_variant_id, qty=float(data.get('quantity', 0.0)), unfolded=True)
                doc['report_type'] = 'pdf'
            ...
            if not candidates:
                if data and data.get('childs'):
                    ...
                    doc = self._get_pdf_line(bom_id,  qty=float(data.get('quantity', 0.0)), unfolded=True)
                doc['report_type'] = 'pdf'
                doc['report_structure'] = data and data.get('report_type') or 'all'
                docs.append(doc)
        return {
