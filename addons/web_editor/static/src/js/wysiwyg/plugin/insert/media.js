...
var AbstractMediaPlugin = AbstractPlugin.extend({
    ...
    _onDblclick: function (e) {
        ...
        if (this.isMediaMethod && this[this.isMediaMethod](archNode)) {
...
var ImagePlugin = AbstractMediaPlugin.extend({
    ...
    get: function (range) {
        return this.isImg(range.scArch) && range.collapse(true);
    },
    /**
     ...
     * @returns {Boolean}
     */
    isImg: function (archNode) {
        return archNode && archNode.isImg();
    },
...
var VideoPlugin = AbstractMediaPlugin.extend({
    ...
    get: function (range) {
        var video = this.isVideo(range.scArch);
        if (video) {
            return range.replace({
                sc: video,
                so: 0
     ...
     * @returns {Boolean}
     */
    isVideo: function (archNode) {
        return archNode.ancestor(function (archNode) {
            return archNode.className && archNode.className.contains('media_iframe_video');
        });
    },
...
var IconPlugin = AbstractMediaPlugin.extend({
    ...
    get: function (range) {
        return this.isIcon(range.scArch) && range.collapse(true);
    },
    /**
     ...
     * @returns {Boolean}
     */
    isIcon: function (archNode) {
        return archNode && archNode.className && archNode.className.contains('fa');
    },
});
...
var DocumentPlugin = AbstractMediaPlugin.extend({
    ...
    get: function (range) {
        return this.isDocument(range.scArch) && range.collapse(true);
    },
    /**
     ...
     * @returns {Boolean}
     */
    isDocument: function (archNode) {
        return archNode && archNode.isAnchor() && archNode.className.contains('o_image');
    },
});
