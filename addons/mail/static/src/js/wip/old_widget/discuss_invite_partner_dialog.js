...
var PartnerInviteDialog = Dialog.extend({
     ...
     */
    init: function (parent, { $thread, store }) {
        const channelName = store.getters['thread/name']($thread);
        this.$thread = $thread;
        this.channelID = store.state.threads[$thread].id;
        this.store = store;
        this._super(parent, {
            title: `Invite people to #${channelName}`,
            size: 'medium',
            buttons: [{
                text: _t("Invite"),
                close: true,
        ...
        this.$input.select2({
            ...
            formatResult: item => {
                ...
                if (item.id === 'odoobot') {
                    ...
                    const $partner = `res.partner_${item.id}`;
                    const partner = this.store.state.partners[$partner];
